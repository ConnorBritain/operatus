...
export function buildConductorOrientationPrompt(input: {
  ...
  return [
    `You are the long-lived Atelier Conductor for Gauntlet Run ${input.runId}.`,
    ...
    'Do not implement. Once the bar is sufficient, freeze it through the Atelier Gauntlet control command.',
    ...
    `Freeze it with: "$HIVE_NODE" "$ATELIER_GAUNTLET_HELPER" freeze --run ${input.runId} --file <contract.json>`
  ].join('\n');
}
...
export function buildWorkerPrompt(input: {
  ...
  const lines = [
    `You are a fresh Atelier ${capitalize(input.role)} for Gauntlet Run ${input.runId}.`,
    ...
    'Produce one or more focused commits, finish with a clean worktree, and report the full HEAD SHA through the Atelier control command.',
    '',
  ...
  lines.push('', `When complete: "$HIVE_NODE" "$ATELIER_GAUNTLET_HELPER" complete --run ${input.runId} --launch ${input.launchId} --sha "$(git rev-parse HEAD)"`);
  return lines.join('\n');
}
...
export function buildCriticPrompt(input: {
  ...
  return [
    `You are the fresh independent Critic for Atelier Gauntlet Run ${input.runId}.`,
    ...
    '',
    'Submit a structured report with the Atelier control command. The report must bind this exact artifact and bar digest.',
    'Write JSON with artifactSha, contractDigest, verdict, summary, and findings to a temporary report file.',
    `Submit it with: "$HIVE_NODE" "$ATELIER_GAUNTLET_HELPER" critic --run ${input.runId} --launch ${input.launchId} --file <report.json>`,
...
export function buildConductorAcknowledgmentPrompt(report: CriticReport): string {
  return [
    ...
    `Acknowledge with: "$HIVE_NODE" "$ATELIER_GAUNTLET_HELPER" acknowledge --run ${report.runId} --file <acknowledgment.json>`
  ].join('\n');
}
