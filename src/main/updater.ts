...
const REPO = 'ConnorBritain/operatus';
...
function autoUpdateEnabled(): boolean {
  try {
    return readConfig().autoUpdate === true; // Operatus-owned releases are opt-in until established
  } catch {
    return false;
  }
}
    ...
    const req = httpsRequest(
      {
        ...
        method: 'GET',
        headers: { 'User-Agent': 'operatus-updater', Accept: 'application/vnd.github+json' },
        timeout: 10_000
      },
      (res) => {
        let body = '';
