...
export interface AgentMeta {
  ...
  cwd: string;
  isGod?: boolean;
  /** Conductor's prep assistant — enriches prompts and forwards them to Conductor.
...
export class HiveManager {
  ...
  ensureHive(): void {
    ...
    // The Claude Code command reference Conductor consults (refreshed each bootstrap
  ...
  private injectedPrompt(meta: AgentMeta, dir: string, root: string, semanticMemory: boolean, knowledgeGraph: boolean): string {
    ...
    const godLine = meta.isGod
      ...
      : meta.isAssistant
      ? 'You are Conductor\'s PREP ASSISTANT. You will be handed short, possibly vague instructions (each begins with "ENRICH TASK:"). For each one: (1) figure out which project it concerns and cd into the most relevant repo — you start in Conductor\'s home directory; (2) gather concrete context READ-ONLY (exact file paths, current state, relevant code, conventions, active branch, gotchas) — NEVER modify, create, or delete files; (3) rewrite the instruction into ONE clear, self-contained prompt that Conductor can execute autonomously, preserving the user\'s original intent without inventing scope. Then deliver it: write ONE message JSON into your outbox with "to":"god", "act":"request", a short subject, and the finished prompt as the body. Do NOT perform the task yourself — your only output is the improved prompt sent to Conductor.'
  ...
  /** Write the live fleet snapshot Conductor reads (`fleet.json`, gitignored).
