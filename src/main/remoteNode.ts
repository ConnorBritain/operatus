...
const DEFAULT_PORTAL_URL = 'https://atelier-pidgeon.vercel.app';
const DEFAULT_SYNC_INTERVAL_MS = 15_000;
export const REMOTE_NODE_SECRET_REF = 'atelier.remote.node-token.v1';
...
export class AtelierRemoteNode {
  ...
  private async executeCommand(command: RemoteCommand): Promise<{
    ...
    try {
      ...
      if (command.operation === 'cancel_run') {
        ...
        const cancelled = this.deps.cancelRun(command.localRunId, 'Cancelled by an authenticated Atelier portal operator');
