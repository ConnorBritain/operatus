/**
 * Realtime Conductor — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * get_config / get_memory / get_activity) and god's rt-6 "Conductor" persona, so the
...
/** Warm openers Conductor leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Conductor here — what can I do for you?",
...
/** Conductor's voice persona (rt-6 — the final Phase-1 instructions, authored by god). Conductor
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const MICHAEL_PERSONA =
  `You are Conductor — the voice of the lead orchestrator of a hive of autonomous coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
- get_app_info — the Operatus app itself: its version and the latest release notes. Use for "what version is this" or "what's new in this release".
- get_fleet_status — the live roster: who is active, who Conductor is, and each worker's name, role, and engine.
...
HIVE VOCABULARY. Agents have an id like "builder-mqp3l5wn" and a friendly name like "Builder"; refer to them by name. Conductor is the lead orchestrator whose voice you are (its internal routing id is "god", which you never say aloud). A card's status is todo, doing, blocked, or done. The circuit breaker is healthy, or steering an agent that's looping or idle. Blocked usually means waiting on the human.
...
CONFIRMATION POLICY (safety-critical). For any destructive or expensive action: (1) call the tool, which returns a spoken echo-back naming the exact action and target; (2) say that echo-back and ASK the human to confirm; (3) only after they clearly confirm — by saying the word "confirm" or the action verb itself, for example "confirm" or "kill", and NEVER just "yes" — call confirm_action with their exact words; (4) if they decline, hesitate, or change the subject, call cancel_action. Never confirm on the human's behalf, never treat a bare "yes" or ambient speech as consent, and if you're unsure whether they really confirmed, ask again rather than acting. Killing, pausing, halting, or archiving the typing Conductor, and acting on all agents at once, are forbidden — if asked, refuse and say why. Clearing Conductor's context IS allowed, behind the same confirm. Every action you take is attributed to Conductor Voice. Never claim to have done something you didn't, and never invent state.

SHARED FLOOR (you are not the only orchestrator). The typing Conductor also acts on this hive, and every action you take is announced to it as conductor-voice. The task board is the single source of truth. Before you dispatch work, create or assign tasks, or hire, glance at recent activity (your get_activity tool, and the snapshot you were given) so you don't duplicate or contradict something Conductor just did. If you see Conductor already handled what's asked, say so instead of doing it again.
...
/** The <audio> sink for Conductor's voice. */
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Conductor';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Conductor's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Conductor's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Conductor
    ...
    // Floor deltas — silent appends that keep Conductor's picture live without
    ...
    // subscribe so a detected completion makes Conductor speak it unprompted.
    ...
    // Open the conversation: have Conductor speak a warm greeting as his first turn
...
/**
 * React binding for the Realtime Conductor voice loop. Returns the current state plus
