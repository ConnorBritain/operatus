...
if (pkg.name !== 'atelier') failures.push(`package name is ${JSON.stringify(pkg.name)}, expected "atelier"`);
if (!['https://github.com/ConnorBritain/atelier.git', 'git+https://github.com/ConnorBritain/atelier.git'].includes(pkg.repository?.url)) {
  failures.push('package repository does not point to ConnorBritain/atelier');
}

const builder = readFileSync(join(root, 'electron-builder.yml'), 'utf8');
for (const expected of ['appId: com.connorbritain.atelier', 'productName: Atelier']) {
...
if (failures.length) {
  ...
  console.log('Atelier identity and redistributable asset checks passed.');
}
