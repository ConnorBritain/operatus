...
### Cross-branch routing and the Corporate layer

Local sockets, provider sessions, agent homes, Hive mailboxes, SQLite, and Git remain inside one branch. A local socket must never be presented as if it spans machines. Cross-machine conversation is nevertheless feasible through the authenticated durable relay already used for projections and narrow commands.

A future workspace-level **Corporate** service sits above branches without becoming their execution authority. It may hold versioned organization policy, a pinned skill catalog, explicitly published knowledge, directory metadata, and sealed work orders. A workspace Conductor can address one branch, a selected set, or all eligible branches. The relay records sender, recipient, expiry, idempotency key, requested capability, content digest, and acknowledgment. Each destination branch's local Conductor then revalidates the message against local policy before delivering it to an agent or starting a venture.

Corporate memory is curated and provenance-bearing, not a silent pool of raw prompts or terminal transcripts. Publishing local knowledge is explicit, scoped, and revocable. Skills are distributed as immutable locks and complete support directories; a branch may reject an unavailable or disallowed skill. Results return as signed summaries and artifact references, while source content remains local unless its sharing policy permits transfer.

This yields a clear authority split:

| Layer | Owns | Does not own |
|---|---|---|
| Corporate workspace | membership, directory, shared policy/skills, durable routing, fleet-wide work orders | provider processes, local files, Git artifacts, run transitions |
| Branch / machine | local Conductor, provider sessions, agent homes, repositories, SQLite and Git authority | another branch's runtime or credentials |
| Client | observation and narrow human commands allowed by identity and role | shell authority or role impersonation |
