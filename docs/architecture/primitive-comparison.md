...
The General Engineering Critic is one fresh review process composed from Atelier's correctness contract, `verification-critic`, and `architecture-reviewer`. The report carries a receipt for each primitive. This composition avoids uncoordinated fanout while preserving independent, typed review concerns.
