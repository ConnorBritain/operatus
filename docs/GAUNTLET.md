...
Atelier creates `atelier/gauntlet/<run-id>` and leaves the final candidate branch unmerged and unpushed. Review or integrate it with ordinary Git tools after the run ends.
...
`gauntlet.db` under Atelier application data is the local protocol authority. Git commits are artifact authority. A report for another SHA or bar digest is stale and cannot pass or repair the current artifact.

The candidate branch is `atelier/gauntlet/<run-id>`. Atelier never merges or pushes it automatically.
...
Atelier discovers and authenticates the CLIs through the inherited Munder runtime. It does not ask for hosted model API keys for Gauntlet execution. If a configured CLI is missing or unauthenticated, the launch fails explicitly and follows the bounded infrastructure-retry policy.
...
Every launch records its actual capability receipt. Atelier does not claim provider parity where the underlying CLI cannot enforce it.

## Role control helper

Atelier-launched roles receive `ATELIER_GAUNTLET_SOCKET`, `ATELIER_GAUNTLET_TOKEN`, and `ATELIER_GAUNTLET_HELPER`. The helper accepts only:

```text
atelier-gauntlet freeze      --run <id> --file contract.json
atelier-gauntlet complete    --run <id> --launch <id> --sha <40-char-sha>
atelier-gauntlet critic      --run <id> --launch <id> --file report.json
atelier-gauntlet acknowledge --run <id> --file acknowledgment.json
atelier-gauntlet escalate    --run <id> --reason <text>
atelier-gauntlet cancel      --run <id> --reason <text>
```
...
- The local authority database is `gauntlet.db` inside the Atelier application profile. Do not edit it directly; inspect through the Runs UI and event stream.
