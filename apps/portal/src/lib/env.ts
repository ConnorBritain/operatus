...
export function getPublicEnv() {
  ...
  if (!url || !publishableKey) {
    throw new Error("Ventura identity is not configured.");
  }
...
export function getServerEnv() {
  ...
  const tokenPepper = process.env.VENTURA_NODE_TOKEN_PEPPER;

  if (!secretKey || !tokenPepper || secretKey === disabledLocalSecret || tokenPepper === disabledLocalSecret) {
    throw new Error("Ventura node control is disabled in this environment.");
  }
