...
export function getPublicEnv() {
  ...
  if (!url || !publishableKey) {
    throw new Error("Atelier identity is not configured.");
  }
...
export function getServerEnv() {
  ...
  const tokenPepper = process.env.ATELIER_NODE_TOKEN_PEPPER;

  if (!secretKey || !tokenPepper || secretKey === disabledLocalSecret || tokenPepper === disabledLocalSecret) {
    throw new Error("Atelier node control is disabled in this environment.");
  }
